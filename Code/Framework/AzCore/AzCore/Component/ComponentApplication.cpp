...
//cjh #include <AzCore/AzCoreModule.h>
...
//cjh #include <AzCore/Component/ComponentApplicationLifecycle.h>
#include <AzCore/Component/TickBus.h>

//cjh #include <AzCore/Debug/LocalFileEventLogger.h>
...
//cjh #include <AzCore/Module/Module.h>
//cjh #include <AzCore/Module/ModuleManager.h>

#include <AzCore/IO/Path/PathReflect.h>
//cjh #include <AzCore/IO/SystemFile.h>

#include <AzCore/Debug/Profiler.h>
//cjh #include <AzCore/Script/ScriptSystemBus.h>
...
//cjh #include <AzCore/UserSettings/UserSettingsComponent.h>
#include <AzCore/Utils/Utils.h>

//cjh #include <AzCore/XML/rapidxml.h>
...
//cjh static void PrintEntityName(const AZ::ConsoleCommandContainer& arguments)
//{
//    if (arguments.empty())
//    {
//        return;
...
//cjh    bool AppDescriptorConverter(SerializeContext& serialize, SerializeContext::DataElementNode& node)
//    {
...
//cjh    struct ProjectPathChangedEventHandler
//    {
...
//        }
    ...
    {
//cjh        DynamicModuleDescriptor::Reflect(context);
...
//cjh        if (AZ::Interface<AZ::Debug::IEventLogger>::Get() == nullptr)
//        {
...
//cjh        // Now that the Allocators are initialized, the Command Line parameters can be parsed
...
//        m_moduleManager = AZStd::make_unique<ModuleManager>();
        ...
        {
            m_console = aznew AZ::Console();//cjh *m_settingsRegistry);
            AZ::Interface<AZ::IConsole>::Register(m_console);
            m_ownsConsole = true;
            m_console->LinkDeferredFunctors(AZ::ConsoleFunctorBase::GetDeferredHead());
//cjh            m_settingsRegistryConsoleFunctors = AZ::SettingsRegistryConsoleUtils::RegisterAzConsoleCommands(*m_settingsRegistry, *m_console);
...
//cjh        m_commandLineUpdatedHandler = {};
...
//        m_projectPathChangedHandler = {};
        ...
        if (m_ownsConsole)
        {
            ...
            delete m_console;
//cjh            ComponentApplicationLifecycle::SignalEvent(*m_settingsRegistry, "ConsoleUnavailable", R"({})");
        }

//cjh        m_moduleManager.reset();
...
//        m_commandLine = {};
    ...
    {
//cjh        AZStd::string errorMessage = {"Unable to determine a valid path to the engine.\n"
...
//cjh        using Type = AZ::SettingsRegistryInterface::Type;
...
//cjh        MergeSettingsToRegistry(*m_settingsRegistry);
    ...
    {
        {
//cjh            AZ::IO::FixedMaxPath outputPath;
...
//cjh        if (m_startupParameters.m_createEditContext)
//        {
...
//cjh        ComponentApplicationLifecycle::SignalEvent(*m_settingsRegistry, "ReflectionManagerAvailable", R"({})");
...
//cjh        Debug::SymbolStorage::RegisterModuleListeners();
#endif // defined(AZ_ENABLE_DEBUG_TOOLS)
        ...
        LoadModules();
//cjh        ComponentApplicationLifecycle::SignalEvent(*m_settingsRegistry, "GemsLoaded", R"({})");
...
//cjh        m_settingsRegistry->Get(platformCachePath.Native(), AZ::SettingsRegistryMergeUtils::FilePathKey_CacheRootFolder);
...
//cjh        AZ::UserSettingsComponentRequestBus::Broadcast(&AZ::UserSettingsComponentRequests::Finalize);
...
//cjh        ScriptSystemRequestBus::Broadcast(&ScriptSystemRequests::GarbageCollect);
...
//cjh        m_moduleManager->DeactivateEntities();
...
//cjh        ComponentApplicationLifecycle::SignalEvent(*m_settingsRegistry, "ReflectionManagerUnavailable", R"({})");
//
...
//cjh        m_budgetTracker.Reset();
#endif

        // Uninit and unload any dynamic modules.
//cjh        m_moduleManager->UnloadModules();
...
//cjh        Debug::SymbolStorage::UnregisterModuleListeners();
#endif // defined(AZ_ENABLE_DEBUG_TOOLS)
    }
...
//cjh    void ComponentApplication::MergeSettingsToRegistry(SettingsRegistryInterface& registry)
//    {
...
//    }
    ...
    // GetSerializeContext
    //=========================================================================
//cjh    AZ::SerializeContext* ComponentApplication::GetSerializeContext()
//    {
...
//    }
    ...
    //=========================================================================
//cjh    AZ::JsonRegistrationContext* ComponentApplication::GetJsonRegistrationContext()
//    {
...
//    }
    ...
    const char* ComponentApplication::GetEngineRoot() const
    {
        ...
        engineRoot.clear();
//cjh        m_settingsRegistry->Get(engineRoot, SettingsRegistryMergeUtils::FilePathKey_EngineRootFolder);
    ...
    const char* ComponentApplication::GetExecutableFolder() const
    {
        ...
        exeFolder.clear();
//cjh        m_settingsRegistry->Get(exeFolder, SettingsRegistryMergeUtils::FilePathKey_BinaryFolder);
...
//cjh        ReflectionEnvironment::GetReflectionManager()->AddReflectContext<SerializeContext>();
        ReflectionEnvironment::GetReflectionManager()->AddReflectContext<BehaviorContext>();
//cjh        ReflectionEnvironment::GetReflectionManager()->AddReflectContext<JsonRegistrationContext>();
    }
    ...
    {
//cjh        if (m_startupParameters.m_createStaticModulesCallback)
//        {
...
//        outModules.emplace_back(aznew AzCoreModule());
    ...
    {
        // Load static modules
//cjh        auto LoadStaticModules = [this](ModuleManagerRequests* moduleManager)
//        {
...
//        ModuleManagerRequestBus::Broadcast(LoadStaticModules);
    ...
    {
//cjh        // Queries the settings registry to get the list of gem modules to load
...
//cjh        if (m_moduleManager->GetSystemComponentTags().empty())
//        {
//            return true;
//        }
//
...
//cjh    AZ::CommandLine* ComponentApplication::GetAzCommandLine()
//    {
...
//cjh        ModuleManager::Reflect(context);
        // reflect descriptor
...
//cjh        if (auto behaviorContext{ azrtti_cast<AZ::BehaviorContext*>(context) }; behaviorContext != nullptr)
//        {
